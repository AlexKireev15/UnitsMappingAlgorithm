...
//}
//
//void Placer::FillDynamicMap(const Vector2f& position)
//{
...
//}
//
//void Placer::SetUnitMapPosition(const std::array<Vector2f, 4u>& rect)
//{
//	if (m_unitDrawables.empty())
//		return;
//
//	Vector2i minIdx(rect[0].x / m_cellSize.x, rect[0].y / m_cellSize.y),
...
//				break;
//
//			std::array<Vector2f, 4u> field =
//			{
//				Vector2f(m_cellSize.x * xdx, m_cellSize.y * ydx),
//				Vector2f(m_cellSize.x * xdx, m_cellSize.y * (ydx + 1u)),
//				Vector2f(m_cellSize.x * (xdx + 1u), m_cellSize.y * (ydx + 1u)),
//				Vector2f(m_cellSize.x * (xdx + 1u), m_cellSize.y * ydx)
//
//			};
//
...
//		Vector2i minIdx(bboxPoints[0].x / m_cellSize.x, bboxPoints[0].y / m_cellSize.y),
...
//{
//	Vector2f dirSum;
//	Vector2f gunDir = pUnitDrawable->GetGunDirection();
//
//	Vector2f defaultGunDir = { 0.f, 1.f };
...
//		{
//			Vector2f position = pBlock->GetPosition();
//			Vector2f dir = position - pUnitDrawable->gun->getPosition();
...
//	}
//	Vector2f ndir = Normalize(dirSum);
//	float angle = GetAngleBetween(gunDir, ndir);
//	pUnitDrawable->RotateGun(angle);
//	pUnitDrawable->SetGunDirection(ndir);
//}
//
//void Placer::MakeHighShadows(const Vector2f& lineupPos, const Vector2f& lineupDir)
//{
...
//			continue;
//		}
//		std::array<Vector2f, 3u> shadowPoints = GetTriangleShadowPoints(lineupPos, lineupDir, pBlock);
...
//
//		Vector2i minIdx(bounds[0].x / m_cellSize.x, bounds[0].y / m_cellSize.y),
...
//}
//
//std::list<Vector2f> Placer::GetWedgeLineup(size_t count, Vector2f boundary, Vector2f padding) const
//{
//	std::list<Vector2f> result;
//
//	float spacing = padding.x;
...
//}
//
//std::list<Vector2f> Placer::GetSquareLineup(size_t count, Vector2f boundary, Vector2f padding, const float& angle) const
//{
//	std::list<Vector2f> result;
//
...
//}
//
//ElementType Placer::GetAreaStatus(const std::array<Vector2f, 4u>& rect) const
//{
//	if (m_unitDrawables.empty())
...
//}
//
//ElementType Placer::GetAreaStatusFast(const std::array<Vector2f, 4u>& rect) const
//{
//	if (m_unitDrawables.empty())
//		return ElementType::FREE;
//
...
//}
//
//Vector2f Placer::FindClosestFreeArea(const std::array<Vector2f, 4u>& rect, const Vector2f& position, const Vector2f& lineupPosition, const Vector2f& direction) const
//{
//	Vector2f origin(-direction.y, direction.x);
//	size_t shiftAbs = 1;
//
//	/*auto insideArea = [this](const Vector2f& p) { return p.x >= 0.f && p.y >= 0.f && p.x <= m_worldSize.x && p.y <= m_worldSize.y; };
//	auto pointsInsideArea = [&insideArea](const std::array<Vector2f, 4u>& points) 
//	{
...
//			Vector2f shiftDir = t * Vector2f(0.f, 1.f);*/
//			Vector2f shiftDir = sign * origin;
//
...
//}
//
//int Placer::GetConnectivity(const Vector2f & pos) const
//{
//	return m_connectivityMap[pos.y / m_cellSize.y][pos.x / m_cellSize.x];
//}
//
//void Placer::FixRotationByBlocks(UnitDrawablePtr& pUnitDrawable, const Vector2f& position, const Vector2f& direction) const
//{
//	for (auto pBlock : m_blocks)
//	{
//		if (!pBlock || pBlock->GetType() != ElementType::BLOCK_HIGH)
...
//
//	auto scalePointsByPadding = [](std::array<Vector2f, 4u>& points, const Vector2f& center, const Vector2f& originalSize, const Vector2f& padding)
//	{
//		sf::Transform scaleT;
